...
export default class CreditCardField extends Component {
  ...
  handleSubmit(event) {
    ...
    if (dateValid)
    {
      this.newUserService.updatePayment([this.state.cardName, this.state.cardNumber, this.state.cardCVV, this.state.cardMonth, this.state.cardYear, this.state.duplicateAddress, this.state.country, this.state.street, this.state.street2, this.state.city, this.state.province], this.state.userID);
    }
    else
    {
